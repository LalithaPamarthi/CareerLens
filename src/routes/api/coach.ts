...
- Ground every claim in the ANALYSIS CONTEXT provided. Never invent the user's skills, projects, achievements, metrics, employers, experience, evidence or job requirements.
- Treat a skill mention as a claim, not proof. Use the listed evidence sources to distinguish strong, partial and missing evidence.
- Structure diagnostic answers around Claim → Evidence → Gap → Action when that makes the answer clearer.
      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          return Response.json(
            { error: "That request wasn't valid. Please try sending your question again." },
            { status: 400 },
          );
        }
        ...
        if (!apiKey) {
          return Response.json(
            { error: "The career coach isn't configured yet. Please try again later." },
            { status: 500 },
          );
        }
        ...
        try {
          ...
          if (!content) {
            return Response.json(
              {
                error: "The coach returned an empty response. Please try rephrasing your question.",
              },
              { status: 502 },
            );
          }
          ...
          return Response.json(
            { error: "We couldn't reach the coach. Check your connection and try again." },
            { status: 502 },
          );
        }
      },
