...
export const Route = createFileRoute("/app/skill-gaps")({
  head: () => ({
    meta: [
      { title: "Evidence gap analysis — CareerLens" },
      {
        name: "description",
        content:
          "Find the skills your target role requires and the evidence your profile still needs.",
      },
      { property: "og:title", content: "Evidence gap analysis — CareerLens" },
      {
        property: "og:description",
        content:
          "Prioritize the missing and partial skill evidence that matters for your target role.",
      },
        ...
        {sorted.map((gap) => (
          <li key={gap.skill} className="surface-card p-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${PRIORITY_CLASS[gap.priority]}`}
              >
            ...
            >
              <div
                className={`h-full rounded-full ${LEVEL_COLOR[gap.level]} ${LEVEL_WIDTH[gap.level]}`}
              />
            </div>
