...
function bulletCandidates(text: string): string[] {
  return lines(text).filter(
    (l) => l.length > 35 && l.split(/\s+/).length > 5 && !/^[A-Z\s]{4,}$/.test(l),
  );
}
...
function hasMetric(line: string): boolean {
  return (
    /\d+\s?(%|percent|x|k\b|ms\b|s\b|users|records|requests|hours|days|weeks|people|students|tests)/i.test(
      line,
    ) || /\$\d/.test(line)
  );
}
...
export function buildAnalysis(inputs: AnalysisInputs): Analysis {
  ...
  const lengthScore = clamp(
    wordCount < 120 ? 35 : wordCount < 250 ? 62 : wordCount > 1200 ? 72 : 88,
  );

  const resumeScore = provided.resume
    ? clamp(
        structureScore * 0.2 +
          metricRatio * 100 * 0.25 +
          verbRatio * 100 * 0.15 +
          keywordOverlap * 100 * 0.2 +
          lengthScore * 0.2,
      )
    : null;
  ...
  const githubScore = provided.github
    ? clamp(
        52 +
          (inputs.repoUrl.trim() ? 14 : 0) +
          Math.min(repoSkills.length * 5, 15) +
          (githubHandle ? 8 : 0),
      )
    : null;
  ...
  const jobMatchScore =
    provided.job && jobSkills.length
      ? clamp(((strongCount + partialCount * 0.5) / jobSkills.length) * 100)
      : null;
  ...
  const overallScore = totalWeight
    ? clamp(
        parts
          .filter(([, v]) => v !== null)
          .reduce((sum, [k, v]) => sum + (v as number) * (weights[k] ?? 0), 0) / totalWeight,
      )
    : 0;
  ...
  if (metricRatio >= 0.35)
    strengths.push({
      id: "s-metric",
      title: "Several bullets show measurable outcomes",
      detail: `${withMetric} of ${resumeBullets.length} bullets include a number, which makes your impact easy to judge.`,
      source: "Resume",
    });
  if (verbRatio >= 0.5)
    strengths.push({
      id: "s-verbs",
      title: "Strong, ownership-led phrasing",
      detail: "Most bullets open with concrete action verbs rather than duties.",
      source: "Resume",
    });
  if (strongCount > 0)
    strengths.push({
      id: "s-match",
      title: "Core role requirements are evidenced",
      detail: `${strongCount} required skill${strongCount === 1 ? "" : "s"} appear in more than one source: ${evidence
  ...
  if (provided.github)
    strengths.push({
      id: "s-github",
      title: "Public code is available for review",
      detail: "A public profile gives reviewers direct evidence beyond the resume.",
      source: "GitHub",
    });
  if (hasLinks && provided.portfolio)
    strengths.push({
      id: "s-links",
      title: "Portfolio includes verifiable links",
      detail: "Live links let a reviewer confirm your work instead of taking it on trust.",
      source: "Portfolio",
    });
  if (!strengths.length)
    strengths.push({
      id: "s-start",
      title: "You have a starting point to work from",
      detail:
        "Add more of your resume, portfolio and target job so the analysis can identify concrete strengths.",
      source: "Provided inputs",
    });
  ...
  if (provided.resume && metricRatio < 0.35)
    weaknesses.push({
      ...
      whyItMatters:
        "Recruiters compare candidates on results. Without numbers, strong work reads the same as routine work.",
      howToFix:
        "Add a real metric to your strongest bullets — users, records, latency, time saved. If you don't have the figure, state the scope instead rather than inventing one.",
    });
  if (weakOpeners.length)
    weaknesses.push({
      ...
      whyItMatters:
        "Passive openers spend the highest-attention words of a line on a duty rather than an achievement.",
      howToFix:
        "Replace with concrete verbs: built, designed, migrated, automated, reduced, shipped.",
    });
  ...
  if (missingRequired.length)
    weaknesses.push({
      ...
      whyItMatters:
        "Unmet required skills are the most common reason a profile is filtered out before a human conversation.",
      howToFix:
        "Pick the one closest to your current work and build a small, documented project that demonstrates it.",
    });
  ...
  if (partials.length)
    weaknesses.push({
      ...
      whyItMatters:
        "A skill listed without project or code evidence reads as a keyword rather than experience.",
      howToFix:
        "Back each one with a project bullet or repository so it appears in at least two places.",
    });
  if (provided.portfolio && !mentionsResult)
    weaknesses.push({
      ...
      whyItMatters:
        "A reviewer spends well under a minute per project, so feature lists hide your actual decisions.",
  ...
  if (!provided.portfolio)
    weaknesses.push({
      ...
      severity: "recommended",
      area: "portfolio",
      problem: "No portfolio information provided.",
      whyItMatters: "Portfolio evidence is where a reviewer verifies what your resume claims.",
      howToFix:
        "Add a portfolio URL or paste your project descriptions and run the analysis again.",
    });
  if (!provided.github)
    weaknesses.push({
      id: "w-nogithub",
      severity: "optional",
      area: "github",
      problem: "No GitHub profile provided.",
      whyItMatters: "Public code is often the fastest credibility signal for technical roles.",
      howToFix: "Add your GitHub profile URL to unlock developer insights.",
    });
  ...
  if (provided.resume && metricRatio < 0.5)
    improvementPlan.push({
      id: "p-metric",
      title: "Add measurable outcomes to your strongest resume bullets",
      horizon: "this-week",
      priority: "high",
      effort: "60–90 minutes",
      impact: "Largest single lift to recruiter perception",
      reason: `Measurable impact is your weakest resume signal (${Math.round(metricRatio * 100)}% of bullets).`,
      action:
        "Use the rewrite suggestions on the Resume page. Real numbers only — state scope if a metric doesn't exist.",
    });
  if (weakOpeners.length)
    improvementPlan.push({
      id: "p-verbs",
      title: "Replace passive bullet openers with action verbs",
      horizon: "this-week",
      priority: "medium",
      effort: "20 minutes",
      impact: "Improves readability and ownership",
      reason: `${weakOpeners.length} bullet(s) currently start with a duty phrase.`,
      action:
        "Rewrite each flagged bullet to start with built, designed, automated, reduced or shipped.",
    });
  if (provided.portfolio)
    improvementPlan.push({
      id: "p-portfolio",
      title: "Rewrite project descriptions with Problem → Role → Stack → Result",
      horizon: "this-week",
      priority: "high",
      effort: "1–2 hours",
      impact: "Makes technical depth visible in a short skim",
      reason: "Descriptions currently emphasise features over context and outcomes.",
      action: "Apply the template on the Portfolio page to each project.",
    });
  if (provided.github)
    improvementPlan.push({
      id: "p-readme",
      title: "Strengthen READMEs on your most relevant repositories",
      horizon: "this-week",
      priority: "medium",
      effort: "45 minutes",
      impact: "Improves credibility of existing work",
      reason: "The README is often the only thing a reviewer reads.",
      action:
        "What it does, how to run it, 3-bullet architecture, one screenshot or sample output.",
    });
  missingRequired.slice(0, 2).forEach((e) =>
    improvementPlan.push({
      id: `p-skill-${e.skill}`,
      title: `Build evidence for ${e.skill}`,
      horizon: "30-days",
      priority: "high",
      effort: "8–12 hours",
      impact: "Closes an unmet requirement for the target role",
      reason: `${e.skill} is required by the job description and has no supporting evidence.`,
      action: `Complete one small, documented project using ${e.skill} and link it from your portfolio.`,
    }),
  );
  partials.slice(0, 2).forEach((e) =>
    improvementPlan.push({
      id: `p-partial-${e.skill}`,
      title: `Add a second evidence source for ${e.skill}`,
      horizon: "30-days",
      priority: "medium",
      effort: "3–6 hours",
      impact: "Turns a keyword into demonstrated experience",
      reason: `Currently found in ${e.sources.join(", ")} only.`,
      action: `Publish or document work that shows ${e.skill} in practice.`,
    }),
  );
  improvementPlan.push({
    id: "p-longterm",
    title: "Deepen one differentiating skill and document it publicly",
    horizon: "60-90-days",
    priority: "medium",
    effort: "20+ hours",
    impact: "Separates you from similar candidates",
    reason: "Depth in one area is more persuasive than shallow coverage of many.",
    action:
      "Pick the highest-priority gap, build a substantial project, and write up the decisions you made.",
  });
  if (!provided.job)
    improvementPlan.push({
      id: "p-job",
      ...
      priority: "high",
      effort: "5 minutes",
      impact: "Unlocks job matching and gap prioritisation",
      reason:
        "Without a target role, recommendations cannot be prioritised against real requirements.",
      action: "Paste the job description on the Analyze page and re-run the analysis.",
    });

  const summaryBits: string[] = [];
  summaryBits.push(
    overallScore >= 75
      ? "Solid foundation overall."
      : overallScore >= 55
        ? "Reasonable foundation with clear gaps."
        : "Early-stage profile with meaningful room to improve.",
  );
  ...
  return {
    ...
    breakdown: [
      {
        ...
        label: "Resume",
        score: resumeScore,
        explanation: provided.resume
          ? `Derived from structure (${sections.length} standard sections), measurable impact (${Math.round(metricRatio * 100)}% of bullets), action verbs and keyword alignment.`
          : "No resume provided.",
      },
      {
        key: "portfolio",
        ...
        explanation: provided.portfolio
          ? `Based on ${portfolioLines.length} description line(s), ${hasLinks ? "verifiable links present" : "no verifiable links"} and ${mentionsResult ? "stated outcomes" : "no stated outcomes"}.`
      ...
      },
      {
        ...
        label: "GitHub",
        score: githubScore,
        explanation: provided.github
          ? "Based on the profile and repository links you provided. Public repository contents are not fetched, so this is an evidence-availability signal."
      ...
      },
      {
        ...
        label: "Job Match",
        score: jobMatchScore,
        explanation: provided.job
          ? `${strongCount} strong, ${partialCount} partial and ${missingRequired.length} missing of ${jobSkills.length} detected requirements.`
        ...
        ? [
            {
              ...
              note: `${sections.length} of 4 standard sections detected.`,
            },
            {
              label: "Measurable impact",
              value: clamp(metricRatio * 100),
              note: `${withMetric} of ${resumeBullets.length} bullets contain a number.`,
            },
            {
              label: "Action verbs",
              value: clamp(verbRatio * 100),
              note: `${verbStarts} bullet(s) open with a strong verb.`,
            },
            {
              label: "Keyword alignment",
              value: clamp(keywordOverlap * 100),
              note: provided.job
                ? `${jobSkills.filter((s) => resumeSkills.includes(s)).length} of ${jobSkills.length} job keywords found.`
    ...
    portfolio: {
        ...
        ? [
            {
              ...
              value: hasLinks ? 85 : 30,
              note: hasLinks
                ? "Portfolio or project links provided."
                : "No live or repository links detected.",
            },
            {
              label: "Description depth",
              value: clamp(Math.min(portfolioText.length / 12, 100)),
              note: `${portfolioLines.length} description line(s) detected.`,
            },
            {
              label: "Stated outcomes",
              value: mentionsResult ? 80 : 30,
              note: mentionsResult
                ? "At least one project states a measurable result."
                : "No measurable results detected in descriptions.",
            },
            {
              label: "Technical signals",
              value: clamp(portfolioSkills.length * 12),
              note: `${portfolioSkills.length} recognised technolog${portfolioSkills.length === 1 ? "y" : "ies"} mentioned.`,
            },
          ]
        : [],
      projects: portfolioLines.slice(0, 4).map((line, i) => ({
        ...
        detected: [
          hasLinks ? "Link available" : "Description text",
          ...(findSkills(line).length ? ["Technology mentioned"] : []),
        ],
        missing: [
          ...(hasMetric(line) ? [] : ["Measurable result"]),
          ...(/\bi\b|\bmy\b|owned|built/i.test(line) ? [] : ["Your specific contribution"]),
        ],
        recommendation:
          "State the problem in one line, what you owned, the stack, and the outcome.",
      })),
      descriptionTemplate: [
    ...
    github: {
      ...
      detected: provided.github
        ? [
            ...
            ...(inputs.repoUrl.trim()
              ? [{ label: "Highlighted repository", value: inputs.repoUrl.trim() }]
              : []),
            ...(repoSkills.length
              ? [
                  {
                    label: "Technologies implied by links",
                    value: repoSkills.map(titleCase).join(", "),
                  },
                ]
              : []),
          ]
      ...
      languages: [],
      highlights: inputs.repoUrl.trim()
        ? [
            {
              name: inputs.repoUrl.trim(),
              detail:
                "Provided as your highlighted repository — make sure its README explains what it does and how to run it.",
            },
          ]
        : [],
        ...
        ? [
            {
              ...
              status: "good",
              note: "A reviewer can reach your code directly.",
            },
            {
              label: "Highlighted repository",
              status: inputs.repoUrl.trim() ? "good" : "warn",
              note: inputs.repoUrl.trim()
                ? "You pointed at a specific repository, which guides the reviewer."
                : "Add one repository you want reviewers to open first.",
            },
            {
              label: "Repository documentation",
              status: "warn",
              note: "Not verifiable from the provided information — check that each highlighted repo has a real README.",
            },
          ]
        : [],
    },
    jobMatch: {
      ...
      preferredSkills: [],
      experienceRequirements: lines(inputs.jobDescription)
        .filter((l) => /\byears?\b/i.test(l))
        .slice(0, 3),
      educationRequirements: lines(inputs.jobDescription)
        .filter((l) => /degree|bachelor|master|b\.?tech|graduat/i.test(l))
        .slice(0, 3),
      responsibilities: lines(inputs.jobDescription)
        .filter((l) =>
          /^(design|build|develop|maintain|collaborate|write|own|work|support|implement)/i.test(l),
        )
        .slice(0, 6),
      evidence,
    },
