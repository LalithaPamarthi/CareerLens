...
function scoreColor(score: number) {
  if (score >= 80) return "[&_[data-slot=progress-indicator]]:bg-success";
  if (score >= 75) return "[&_[data-slot=progress-indicator]]:bg-primary";
...
export function ScoreCard({
  ...
  return (
    <div
      className={cn(
        "surface-card flex flex-col gap-3 p-5 transition-shadow hover:shadow-[var(--shadow-card)]",
        className,
      )}
    >
      ...
      {score === null ? (
        <p className="text-sm text-muted-foreground">
          {emptyHint ?? "Not enough information provided yet."}
        </p>
      ) : (
        <>
          ...
          <Progress value={score} aria-label={`${label} score`} className={scoreColor(score)} />
        </>
      )}
